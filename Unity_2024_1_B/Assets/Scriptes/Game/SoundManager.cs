...
{
    public static SoundManager intance;         //싱글톤 인스턴스 화 시킨다.

    public List<Sound> sounds = new List<Sound>();          //사운드 리스트 (배열보다 유연한 자료도구)
    ...
    private void Awake()
    {
        ...
        foreach (Sound sound in sounds)              //리스트 안에 있는 사운드들을 초기화 한다.
        {
    ...
    //사운드를 재생하는 메서드
    public void PlaySound(string name)                  //인수 Name받아서
    {
        Sound soundToPlay = sounds.Find(sound => sound.name == name);       //List 안에있는 name 이 같은것을 검색 후 soundToPlay에 선언
