...
public class ExPlayer : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            m_Rigidbody.AddForce(transform.up * Force);      //tuansform.up(위쪽 방향)으로 리지드 바디에 Force 힘만큼 물리의 힘을 준다.
        }
    ...
    private void OnCollisionEnter(Collision collision)      //충돌이 시작되었을때
    {
        if (collision != null)                               //충돌 물체가 없지 않을 경우 [ != 다를 경우]
        {
            point = 0;                                      //충돌이 일어났을때 포인트를 0으로 해분자.
